...
import { ResultCheckEligibleDto ,EligibleEpisodeListDto ,FindPatientInfoResultInfo ,FindEpisodeInfoResultInfo
  ...
  CreateTransactionDto ,PolicyNumberListDto ,FindPolicyNumberInfo
...
async getListPolicyNo(queryEligibleBodyDto : QueryEligibleBodyDto){
    let RequesetBody;
    let newPolicyNumberListDto= new PolicyNumberListDto();
    let newFindPolicyNumberInfo: FindPolicyNumberInfo[]
    try{
   RequesetBody ={
    xRefID:queryEligibleBodyDto.PatientInfo.RefId||'',
  }
  ...
  async getListPolicyNoDetail(queryEligibleBodyDto : QueryEligibleBodyDto){
    let RequesetBody;
    let newPolicyNumberListDto= new PolicyNumberListDto();
    let newFindPolicyNumberInfo: FindPolicyNumberInfo[]
    try{
  ...
  return xRefId
   ...
   async ListPolicyNumber(insuranceData: InsuranceData, inputhn: string, inputvn: string) {
  if (!insuranceData?.CoverageList) return;
  ...
  // ✅ รวมข้อมูลจากทุก coverage
  for (const coverage of insuranceData.CoverageList) {
    const type = coverage.Type || '';
    const messages = coverage.MessageList || [];

    for (const message of messages) {
      // ✅ ข้ามถ้า PolicyNo ว่าง หรือ RuleNo = 'NOT_PASS'
      if (!message.PolicyNo || message.RuleNo === 'NOT_PASS') continue;
  ...
  // ✅ ตรวจสอบว่ามีข้อมูลจะ insert หรือไม่
  if (result.length > 0) {

    await prismaProgest.policynumbertransactions.deleteMany({
      where: {
        refid: insuranceData.RefId,
      },
    });
    // ✅ กำจัดข้อมูลซ้ำโดยใช้ PolicyNo + RuleNo + Type เป็น key
    const uniqueResult = uniqBy(result, (item) => `${item.PolicyNo}-${item.RuleNo}-${item.Type}`);
    ...
    //console.log('--9999---')
    await prismaProgest.policynumbertransactions.createMany({
      data: insertData,
      skipDuplicates: true, // ป้องกัน insert ซ้ำ (กรณีมี unique constraints ใน DB)
    });
  }
}
}
