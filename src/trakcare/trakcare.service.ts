...
async getPreAuthBilling( xVN: string ) {
  let response:any ;
  let PatientInfo ;
  try{
     response = await firstValueFrom(
      this.httpService.get(`${TRAKCARE_APIURL}/getPreAuthBilling/${xVN}`)
    );
    PatientInfo = response.data
  } catch(error)
    {
...
async getBillingSubgroup( xBillingCode: string ) {
  let response:any ;
  let PatientInfo ;
  try{
     response = await firstValueFrom(
      this.httpService.get(`${TRAKCARE_APIURL}/getBillingSubgroup/${xBillingCode}}`)
    );
    PatientInfo = response.data
  } catch(error)
    {
